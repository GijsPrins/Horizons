  ...
  <DefaultLayout>
    <v-container fluid class="pa-4">
      ...
      <div class="d-flex align-center mb-4">
        <div>
          ...
          <p class="text-body-2 text-medium-emphasis">
            Beheer je teams en nodig anderen uit
          </p>
        </div>
      ...
      >
        ...
        <h2 class="text-h6 mb-2">Nog geen teams</h2>
        <p class="text-body-2 text-medium-emphasis mb-4">
          Maak een team aan of join er een met een uitnodigingscode.
        </p>
      ...
      <v-dialog v-model="createDialogOpen" max-width="500">
        <v-card>
          <v-card-title>Team aanmaken</v-card-title>
          <v-card-text>
            <v-form ref="createFormRef" @submit.prevent="handleCreate">
              <v-text-field
                v-model="createForm.name"
                label="Teamnaam"
                placeholder="Bijv. Familie Doelen 2025"
              ...
              <v-textarea
                ...
                placeholder="Waar gaat dit team over?"
                rows="2"
                auto-grow
              />
            </v-form>
          ...
          <v-card-actions>
            <v-spacer />
            <v-btn variant="text" @click="createDialogOpen = false">Annuleren</v-btn>
            <v-btn
              color="primary"
              variant="flat"
              :loading="isCreating"
      ...
      <v-dialog v-model="joinDialogOpen" max-width="400">
        <v-card>
          <v-card-title>Team joinen</v-card-title>
          <v-card-text>
            <p class="text-body-2 text-medium-emphasis mb-4">
          ...
          <v-card-actions>
            <v-spacer />
            <v-btn variant="text" @click="joinDialogOpen = false">Annuleren</v-btn>
            <v-btn
              color="primary"
              variant="flat"
              :loading="isJoining"
...
const { teams, isLoading, createTeam, joinTeam, isCreating, isJoining } = useTeams()
...
async function handleCreate() {
  ...
  try {
    ...
    showSnackbar?.('Team aangemaakt!', 'success')
